...
import CabinetRelated from '~icons/mdi/file-cabinet'
import Quotation from '~icons/mdi/file-document-outline'
...
async function processMenuOptions(options, t) {
  ...
  const processedOptions = await Promise.all(options.map(async (option) => {
    ...
    const { key } = option
    const iconModule = await import('~icons/mdi/function-variant')
    const icon = await renderMenuIcon(iconModule.default, 16) // 调用 renderIcon 处理图标大小
    let children = null
...
//   },
//   {
//     label: renderMenuLabel(() => t('MENU.CabinetRelated')),
//     key: 'cabinet-related',
//     icon: renderMenuIcon(CabinetRelated),
//     children: [
//       {
//         label: renderMenuLabel(() => t('MENU.Quotation')),
//         key: 'Quotation',
//         icon: renderMenuIcon(Quotation)
//       }
//     ]
//   },
//   {
